...
struct input {
    struct {
        ...
        lv_indev_state_t encoder_button;
    } mouse;
...
void wayland_init(void)
{
    ...
    if (application.format == 0xFFFFFFFF) {
        fprintf(stderr, "WL_SHM_FORMAT not available\n");
        return;
    }
    ...
    if (!path) {
        fprintf(stderr, "Cannot get XDG_RUNTIME_DIR: %s\n", strerror(errno));
        return;
    }

    name = malloc(strlen(path) + sizeof(template));
    if (!name) {
        fprintf(stderr, "Cannot malloc name: %s\n", strerror(errno));
        return;
    }
    ...
    if (fd >= 0) {
        ...
        if ((flags == -1) || (fcntl(fd, F_SETFD, flags | FD_CLOEXEC) == -1)) {
            fprintf(stderr, "Cannot set FD_CLOEXEC\n");
            close(fd);
            fd = -1;
        }
        unlink(name);
    ...
    if (fd < 0) {
        fprintf(stderr, "Cannot create tmpfile: %s\n", strerror(errno));
        return;
    }
 ...
 * @param data where to store input data
 */
bool wayland_encoder_read(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    data->state = application.input.mouse.encoder_button;
    data->enc_diff = application.input.mouse.encoder_diff;

    application.input.mouse.encoder_diff = 0;
...
static void seat_handle_capabilities(void *data, struct wl_seat *wl_seat, enum wl_seat_capability caps)
{
    ...
    if ((caps & WL_SEAT_CAPABILITY_POINTER) && !seat->wl_pointer) {
        ...
        wl_pointer_set_user_data(seat->wl_pointer, &application);
    ...
    if ((caps & WL_SEAT_CAPABILITY_KEYBOARD) && !seat->wl_keyboard) {
        ...
        wl_keyboard_set_user_data(seat->wl_keyboard, &application);
    ...
    if ((caps & WL_SEAT_CAPABILITY_TOUCH) && !seat->wl_touch) {
        ...
        wl_touch_set_user_data(seat->wl_touch, &application);
                                  ...
                                  uint32_t state)
{
    ...
    switch (button & 0xF) {
    case 0:
        app->input.mouse.left_button =
            (state == WL_POINTER_BUTTON_STATE_PRESSED) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
        break;
    case 1:
        app->input.mouse.right_button =
            (state == WL_POINTER_BUTTON_STATE_PRESSED) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
        break;
    case 2:
        app->input.mouse.encoder_button =
            (state == WL_POINTER_BUTTON_STATE_PRESSED) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
        break;
    default:
        break;
                                ...
                                uint32_t time, uint32_t axis, wl_fixed_t value)
{
    ...
    if (axis == 0) {
        app->input.mouse.encoder_diff = wl_fixed_to_int(value);
    }
}
                                   ...
                                   uint32_t format, int fd, uint32_t size)
{
    ...
    xkb_keymap_unref(app->input.keyboard.xkb.keymap);
    xkb_state_unref(app->input.keyboard.xkb.state);
    app->input.keyboard.xkb.keymap = keymap;
    app->input.keyboard.xkb.state = state;
}
                                ...
                                uint32_t state)
{
    ...
    if (!app->input.keyboard.xkb.state)
        return;

    num_syms = xkb_state_key_get_syms(app->input.keyboard.xkb.state, code, &syms);
    if (num_syms == 1) {
        sym = syms[0];
    } else {
    ...
    lv_key = keycode_xkb_to_lv(sym);
    if (lv_key != 0) {
        app->input.keyboard.key = lv_key;
        app->input.keyboard.state =
                                      ...
                                      uint32_t group)
{
    ...
    if (!app->input.keyboard.xkb.keymap)
        return;

    xkb_state_update_mask(app->input.keyboard.xkb.state,
...
static lv_key_t keycode_xkb_to_lv(xkb_keysym_t xkb_key)
{
    ...
    if (((xkb_key >= 0x0020) && (xkb_key <= 0x007e))) {
        key = xkb_key; 
    } else {
        switch (xkb_key)
        {
